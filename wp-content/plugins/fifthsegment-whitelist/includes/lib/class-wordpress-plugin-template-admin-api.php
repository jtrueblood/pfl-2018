...
// print_r($post);
		...
		if ( $post ) {
			...
			if (isset($xdata["field"]['display_if'])){

				$new_option =  $xdata['prefix'];
				// $new_option .= "wpt_";
				$new_option = $new_option.$xdata["field"]['display_if'];
				// print $new_option;
				// $this_option = $option_name.$xdata['field']['prefix'];
				$this_option = $xdata['prefix'].$xdata['field']['id'];

					$val = get_option( $new_option );
					if ($val == 'on'){
						print $xdata["field"]["display_if_msg"];
						update_option( $this_option, '' );
						return;
					}
