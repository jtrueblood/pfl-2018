    ...
    try:
        with open(csv_file_path, 'r') as f:
            ...
            for values in reader:
                ...
                # Passing: Cmp(14), Att(15), Yds(18), TD(19), Int(20)
                # Rushing: Att(33), Yds(34), Y/A(35), TD(36), 1D(37), Succ%(38)
                
                # Use column indices for stats to handle duplicate names
                pass_yds = safe_int(values[18]) if len(values) > 18 else 0
                pass_td = safe_int(values[19]) if len(values) > 19 else 0
                pass_int = safe_int(values[20]) if len(values) > 20 else 0
                
                rush_yds = safe_int(values[34]) if len(values) > 34 else 0
                rush_td = safe_int(values[36]) if len(values) > 36 else 0
