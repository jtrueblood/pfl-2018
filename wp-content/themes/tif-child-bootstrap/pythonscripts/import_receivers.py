#!/usr/bin/env python3
"""
Import Wide Receiver CSV data from Pro Football Reference into wp_stathead_WR MySQL table.
"""
...
def extract_year_from_filename(filepath):
    """Extract year from filename like '1991-WR.csv'"""
...
def import_csv_to_db(csv_file_path):
    """Import CSV file into wp_stathead_WR table"""
    ...
    if not year:
        ...
        print("Expected format: YYYY-WR.csv (e.g., 1991-WR.csv)")
        return False
    ...
    try:
        with open(csv_file_path, 'r') as f:
            ...
            for values in reader:
                ...
                # WR CSV column positions (similar to RB):
            ...
            # Confirm import
            response = input(f"\n⚠️  Import {len(rows_to_insert)} rows into wp_stathead_WR? (yes/no): ").strip().lower()
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 import_receivers.py <path_to_csv>")
        print("Example: python3 import_receivers.py ../pfr-raw-season/1991-WR.csv")
        sys.exit(1)
    
    csv_file = sys.argv[1]
    
    print("=" * 60)
    print("WIDE RECEIVER CSV IMPORTER")
    print("=" * 60)
