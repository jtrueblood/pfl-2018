<?php
/*
 * Template Name: Tables Postseason Page
 * Description: Page for displaying postseason data tables
 */
 ?>
...
<div id="content-container">
	...
	<div id="page-content">
		...
		<div class="row">
			...
			<div class="col-xs-24">
				<h4>Postseason Data - Teams & Players</h4>
				...
				<p><a href="/tables">View All Tables</a> | <a href="/tables-teams">Team Tables</a> | <a href="/tables-players">Player Tables</a> | <a href="/tables-other">Other Tables</a></p>
			<!-- NEW SECTION -->
			<div class="col-xs-24">
			...
			<div class="col-xs-24">
				...
				<!-- Postseason-Only Players -->
				<div class="col-xs-24 col-sm-12 col-md-6">
					<?php
					// Get all unique player IDs from wp_playoffs
					$all_playoff_players = $wpdb->get_col("SELECT DISTINCT playerid FROM wp_playoffs");
					
					// Loop through and check regular season games count
					$postseason_only = array();
					foreach ($all_playoff_players as $player_id) {
						if (empty($player_id) || $player_id == 'None') {
							continue;
						}
						
						$career_stats = get_player_career_stats($player_id);
						$games = isset($career_stats['games']) ? $career_stats['games'] : 0;
						
						// If 0 regular season games, add to postseason_only array
						if ($games == 0) {
							$postseason_only[] = $player_id;
						}
					}
					
					// Get their total postseason points and team info
					$postseason_only_players = array();
					foreach ($postseason_only as $player_id) {
						$total_points = $wpdb->get_var($wpdb->prepare(
							"SELECT SUM(points) FROM wp_playoffs WHERE playerid = %s",
							$player_id
						));
						
						$games_played = $wpdb->get_var($wpdb->prepare(
							"SELECT COUNT(*) FROM wp_playoffs WHERE playerid = %s",
							$player_id
						));
						
						// Get team(s) they played for - use DISTINCT in case they played for multiple teams
						$teams = $wpdb->get_col($wpdb->prepare(
							"SELECT DISTINCT team FROM wp_playoffs WHERE playerid = %s",
							$player_id
						));
						$team_list = implode(', ', $teams);
						
						// Check if they won a championship (week 16, result = 1)
						$won_championship = $wpdb->get_var($wpdb->prepare(
							"SELECT COUNT(*) FROM wp_playoffs WHERE playerid = %s AND week = 16 AND result = 1",
							$player_id
						));
						
						// Include all postseason-only players, even if they scored 0 points
						$postseason_only_players[$player_id] = array(
							'points' => $total_points ? $total_points : 0,
							'games' => $games_played,
							'team' => $team_list,
							'champion' => $won_championship > 0 ? 'Yes' : 'No'
						);
					}
					
					// Sort by total points
					uasort($postseason_only_players, function($a, $b) {
						return $b['points'] - $a['points'];
					});
					
					$labels = array('Player', 'Team', 'Games', 'Points', 'Champion');	
					tablehead('Postseason-Only Players', $labels);
					
					$postseason_only_print = '';
					foreach ($postseason_only_players as $player_id => $stats) {
						$name = get_cached_player_name($player_id);
						
						// Show trophy icon for champions, nothing for non-champions
						$champion_display = ($stats['champion'] == 'Yes') ? '🏆' : '';
						
						$postseason_only_print .= '<tr><td>' . $name['first'] . ' ' . $name['last'] . '</td>';
						$postseason_only_print .= '<td class="text-center">' . $stats['team'] . '</td>';
						$postseason_only_print .= '<td class="text-center">' . $stats['games'] . '</td>';
						$postseason_only_print .= '<td class="text-right">' . $stats['points'] . '</td>';
						$postseason_only_print .= '<td class="text-center">' . $champion_display . '</td></tr>';
					}
					
					if (empty($postseason_only_print)) {
						$postseason_only_print = '<tr><td colspan="5" class="text-center">No players found</td></tr>';
					}
					
					echo $postseason_only_print;
					
					tablefoot('-- Never played regular season');	
					?>
				</div>
